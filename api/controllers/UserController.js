...
module.exports = {
	...
	fixtureUser:function(req,res) {
		...
		for(var i in dataUsers.data){
			var thisdata = {};
			thisdata.name = dataUsers.data[i][0];
			thisdata.pseudo = dataUsers.data[i][1];
			thisdata.email = dataUsers.data[i][2];
			thisdata.phone = dataUsers.data[i][3];
			thisdata.company = dataUsers.data[i][4];
			thisdata.role = dataUsers.data[i][5];
			thisdata.publishEmail = dataUsers.data[i][6];
			thisdata.publishPhone = dataUsers.data[i][7];
			thisdata.usename = dataUsers.data[i][8];
			thisdata.password = dataUsers.data[i][9];
			thisdata.civ = dataUsers.data[i][10];
			thisdata.createdAt = new Date(Number(dataUsers.data[i][12])*1000);
			thisdata.lastActivity = new Date(Number(dataUsers.data[i][13])*1000);
			console.log(dataUsers.data[i][5]);
			if(dataUsers.data[i][5] == 'member'){
				console.log('here');
				console.log();

				thisdata.dateMember = new Date(Number(dataUsers.data[i][11])*1000);
			}
