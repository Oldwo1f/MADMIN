app.factory('documentService', ['$http', '$q', function ($http,$q) {
    ...
    service.fetch= function(stateParams) {
        ...
        $http({method:'get',url:'/document/fetch',params:service.filter}).success(function (data,status) {
            console.log(data);
            service.list =data;
    ...
    service.update=function(img){
        ...
        var deferred = $q.defer();
        $http.put('/document',img).success(function (data2,status2) {
            console.log('SUCCESS');
            console.log(data2);
            deferred.resolve(data2);
    ...
    service.remove=function(id){
        ...
        var deferred = $q.defer();
        // user.role = 'user'
        $http.delete('/document/'+id).success(function (data2,status2) {
            console.log('SUCCESS');
            console.log(data2);
            deferred.resolve(data2);
